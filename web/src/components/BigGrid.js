...
export function BigGrid(props) {
  ...
  const getColumnWidth = ({ index }) => {
    const column = props.headers[index];
  ...
  return (
    <DataContainer>
      <DraggableHeaders
        key={_.join(props.headers, ',')} // the key is cat of all columns, re-mount if any col changes
      ...
      <BodyContainer ref={bodyContainerRef}>
        <Grid
          ...
          cellRenderer={(cellRenderProps) => {
            const Renderer = props.renderers[cellRenderProps.columnIndex];
            return (
              <div
              ...
              >
                <Renderer index={cellRenderProps.rowIndex} />
              </div>
            );
            // const result = (c) => {
            //   return (
...
function DraggableHeader(props) {
  ...
  return (
    <div
      ...
      ref={elementRef}
      style={{
        height: `${Sizes.DefaultRowHeight}px`,
        display: 'flex',
